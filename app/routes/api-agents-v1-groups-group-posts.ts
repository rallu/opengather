...
export async function createAgentGroupPost(params: {
	request: Request;
	groupId: string;
	...
	const parsedBody = await readAgentJsonBody<AgentPostPayload>({
		request: params.request,
		requestId,
	});
	if (!parsedBody.ok) {
	...
	const db = (params.db ??
		(await import("../server/db.server.ts")).getDb()) as AgentPostDb;
	...
	const canPost = canSubjectPostToGroup({
